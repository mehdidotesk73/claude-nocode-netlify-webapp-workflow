...
One caveat: skills added to a repo mid-session don't register until the session ends and a new one starts — confirmed against Claude Code's own docs (`/clear`, `/compact`, `/mcp` are the only in-session context commands, and none re-scan `.claude/skills/`; there's no reload/restart command at all). So the bootstrap tells Claude to read `.claude/skills/finish-setup/SKILL.md` directly if the user chooses to stay in the current session rather than start a fresh one. That same fact is also why Step 4 (copy/transform/build/push) can't itself be a skill: it's the step that writes `.claude/skills/` into the new repo in the first place, so there's nothing to invoke until after it runs, and nothing short of a new session would make it invocable even then.

**A "read the file directly if it's not discoverable" fallback quietly defeats the point of moving setup into skills at all.** Skills are discovered once at session start, with no re-scan mechanism — this was worth checking against the actual docs rather than assuming, since inventing a plausible-sounding `/reload` would have been worse than admitting uncertainty. There isn't one. If this session never ends, `ship-feature` doesn't just fail to trigger once — it's unavailable for every remaining turn in the conversation, for every future feature. Falling back to "just read the file and follow it" reintroduces exactly the failure mode this whole restructuring was meant to close: a procedure that depends on Claude remembering to go look, rather than one the harness enforces by triggering it.
